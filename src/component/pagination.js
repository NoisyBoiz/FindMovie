...
function Pagination({totalPages, indexPage, setIndexPage, showPagination}){
    ...
    return(
        <div className="bottomCommonContainer">
                {showPagination? <>
                    <button className="prePageCommon" disabled={indexPage<=1} onClick={()=>{changePage(indexPage-1,totalPages)}}><i className="fa-solid fa-chevron-left"></i></button>
                        <div className="indexPageCommon"> 
                            {arrButPage.length&&arrButPage.map((item,index)=>{
                                return <button key={index} className={item.index===indexPage?"focusButtonCommon":"unfocusButtonCommon"} onClick={()=>{changePage(item.index,totalPages)}}> {item.sign} </button>
                            })}
                        </div>
                        <button className="nextPageCommon" disabled={indexPage>=totalPages} onClick={()=>{changePage(indexPage+1,totalPages) }}><i className="fa-solid fa-chevron-right"></i></button>
                        <div className="inputPageCommon"> <input type="number" value={inputPage} onChange={(e)=>{setInputPage(e.target.value);}} onKeyDown={(e)=>{if(e.key==="Enter"){if(Number(inputPage)>0&&Number(inputPage)<=totalPages){changePage(Number(inputPage),totalPages)}}}}/> <span> / {totalPages} </span> 
                    </div>
                </>
                :
                indexPage<totalPages?
                <button className="buttonMoreCommon" onClick={()=>{setIndexPage(indexPage+1)}}> 
                    <span> {t("More")} </span>
